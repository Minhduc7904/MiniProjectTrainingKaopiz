---
name: devkit-spec
description: "OpenSpec entry (SBU2 runtime): route via sbu2.openspec-router and kaopiz-devkit openspec — full pipeline parity, no /opsx slash in consumer projects."
---

> DevKit agent command `devkit-spec`, published as a skill because Codex has no repo-level slash commands.
...
1. Load and follow **`sbu2.openspec-router`** (Read `.agents/skills/sbu2.openspec-router/SKILL.md` or vendor path in the wrapper).
...
If you already use SBU2/OpenSpec slash names, **`devkit-sync`** copies `/opsx`, `/opsx-apply`, `/opsx-brainstorm`, etc. into `.agents/commands/` when `runtime_engine=sbu2-ai-kit`. Each file is prefixed to route through the same vendored **`sbu2.*`** skills as native OpenSpec.
