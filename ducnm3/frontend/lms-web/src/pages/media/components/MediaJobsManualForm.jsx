...
export function MediaJobsManualForm({ query, loading, onChange, onSubmit }) {
  return <form className="flex h-full min-h-0 flex-col" onSubmit={(event) => { event.preventDefault(); onSubmit(query) }}>
    <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
      <p className={`mb-4 text-[13px] ${adminUi.body}`}>{UI_LABELS.queryOnly}</p>
    ...
    <div className={`shrink-0 px-5 py-3 ${adminUi.hairlineT}`}><Button type="submit" disabled={loading}>{UI_LABELS.callApi}</Button></div>
  </form>
}
