...
public static class CreateCourseEndpoint
{
    public static RouteHandlerBuilder MapCreateCourse(this IEndpointRouteBuilder endpoints) =>
        ...
        {
            var result = await handler.HandleAsync(request.Name, request.DescriptionMarkdown, request.Status, ReadActor(context.Request), cancellationToken);
        ...
        }).WithName("create-course").WithTags(ServiceNames.Course).Accepts<CreateCourseRequest>("application/json").Produces<ApiResponse<CourseCommandResponse>>(StatusCodes.Status201Created);
    ...
    internal static Guid ReadActor(HttpRequest request)
    {
        return Guid.TryParse(request.Headers[ApiHeaderNames.ActorId], out var actor) && actor != Guid.Empty
            ? actor : throw new ArgumentException("Actor headers are required.");
    }
}

public sealed record CreateCourseRequest(string? Name, string? DescriptionMarkdown, string? Status);
