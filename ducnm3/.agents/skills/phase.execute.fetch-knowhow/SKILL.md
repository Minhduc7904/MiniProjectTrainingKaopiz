...
|---------------|-------------------------------|---------------|
| HTTP APIs, DTOs, handlers | Whatever your Hub exposes for API/security (often `auth`, payment-related domains) | Use **`PLAN.md`** + repo rules (`.agents/rules`, ADRs). Do **not** skip security-sensitive changes without written rules. |
