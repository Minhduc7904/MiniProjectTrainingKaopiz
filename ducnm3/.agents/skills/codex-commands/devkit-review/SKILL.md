---
name: devkit-review
description: "Review code changes on the current branch using the code-review-excellence skill (4-phase process). Produces a structured review report using the PR review template. Output is designed to be included…"
---

> DevKit agent command `devkit-review`, published as a skill because Codex has no repo-level slash commands.
...
The **`code-review-excellence`** skill must be available. It is automatically pulled from GitHub during `devkit:sync` (or `kaopiz-devkit sync`). Confirm by checking that the skill exists under `.agents/skills/code-review-excellence/` or `.claude/skills/code-review-excellence/`.
...
This file syncs to **`.agents/commands/`** and **`.claude/commands/`** (Claude Code).
