...
| Media usage policy | `HandleAsyncThumbnailDerivativeForInternalMediaTupleThrowsInvalidMedia` | Admin gửi direct tuple `MEDIA/MEDIA_THUMBNAIL/THUMBNAIL` với media derivative WebP. | Handler trả `INVALID_MEDIA`; thumbnail derivative chỉ được Worker liên kết với original. |
...
| Media Library | `HandleAsync_StatusIsLowercase_NormalizesAndPassesFilterToRepository` | Actor gửi `status=pending`. | Handler normalize thành `PENDING` và truyền filter đã kiểm tra allowlist xuống repository. |
...
| Media background job list | `GetMediaBackgroundJobsHandlerTests` | ADMIN gửi job type/status/correlation ID hợp lệ hoặc status không allowlist. | Filter được normalize, offset total pages đúng; status sai trả `INVALID_MEDIA_JOB_QUERY`. |
| Markdown/delete background lifecycle | Markdown sync hoặc batch xóa usage chạy thành công/thất bại. | Consumer ghi PROCESSING trước side effect, COMPLETED sau thành công; fault consumer ghi FAILED và lỗi an toàn sau khi retry hết. |
