---
name: devkit-workflow-profile
description: "Configure devkit.workflow-profile.json: interview answers, init steps, enabled phases; CLI infer, interview, template."
---

> DevKit agent command `devkit-workflow-profile`, published as a skill because Codex has no repo-level slash commands.
...
**Required** after adding/changing commands in the Hub: `npm run devkit:sync` so `.agents/commands/` stays up to date.
