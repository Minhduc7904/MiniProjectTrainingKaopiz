// File: backend/Services/Media/MediaService.UnitTests/Api/Endpoints/Media/MediaCommandEndpointTests.cs
// Mục đích: Kiểm thử HTTP contract, route và response của endpoint MediaCommandEndpointTests.
...
public sealed class MediaCommandEndpointTests
{
    ...
    public async Task MultipartUploadAndUsageReturnStandardCreatedResponses()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(uploadBody, Does.Contain("\"isDraft\":true"));
            Assert.That(
                uploadBody,
                Does.Contain("\"draftedAtUtc\":\"2026-08-18T03:04:05.678Z\""));
            Assert.That(
                uploadBody,
