---
name: kaopiz-devkit-skill-authoring
description: "Build a markdown prompt to author SKILL.md via skill-authoring prompt; run scan after write."
---

> DevKit agent command `kaopiz-devkit-skill-authoring`, published as a skill because Codex has no repo-level slash commands.
...
**After writing `SKILL.md`:** Cursor rule **`kaopiz-devkit-skill-scan-after-write`** (glob `.agents/skills/**/SKILL.md`) requires the Agent to run **`skill-authoring scan`** — see **REQUIRED** below.
...
5. **Scan after `SKILL.md` exists:** see **REQUIRED** above. You may combine: **`… skill-authoring prompt -m "…" --scan .agents/skills/<id>`** if the path is known upfront.

6. **Optional** (only if the user explicitly asks): from the prompt, draft **`SKILL.md`** at a valid path (e.g. `.agents/skills/<id>/SKILL.md` or Hub staging — repo-dependent).
