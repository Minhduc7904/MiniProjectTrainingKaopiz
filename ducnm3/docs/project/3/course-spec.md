...
| Alternative flow | Khóa học đã ở `DRAFT` được tiếp tục cập nhật trước khi xuất bản; thao tác lưu bản nháp không yêu cầu đủ điều kiện xuất bản. |
...
| Alternative flow | Khi cập nhật Bài học mà `display_order` không đổi hoặc vị trí mới chưa được dùng, hệ thống cập nhật bản ghi hiện tại thay vì tạo Bài học mới. |
...
| Alternative flow | Bộ lọc hợp lệ nhưng không có Khóa học phù hợp trả danh sách rỗng cùng metadata phân trang hợp lệ. |
...
| Alternative flow | Yêu cầu hợp lệ được gửi lại cho lượt ghi danh đã tồn tại trả kết quả idempotent và không tạo bản ghi thứ hai. |
| Error cases | Chưa xác thực; Học viên không đủ điều kiện; Khóa học không tồn tại hoặc chưa `PUBLISHED`. |
...
| Cases | Happy: ghi danh mới. Boundary: Khóa học vừa đổi trạng thái, request lặp. Negative: student inactive, course DRAFT, course không tồn tại. |
...
| Alternative flow | Gửi lại mức 100% cho Lesson đã hoàn thành giữ nguyên một bản ghi tiến độ và không tạo thêm thời điểm hoàn thành mới. |
...
| Alternative flow | Bộ lọc hợp lệ nhưng không có record phù hợp trả tệp CSV chỉ có header; dataset lớn được stream/chunk thay vì materialize toàn bộ. |
