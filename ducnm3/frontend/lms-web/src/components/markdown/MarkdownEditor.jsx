...
export function MarkdownEditor({ value, onChange, disabled = false, placeholder = 'Nội dung Markdown...', id = 'markdown-content' }) {
  ...
  return (
    <div className={`rounded-lg p-4 ${ui.card}`}>
      <div className="flex items-center justify-between gap-3"><div><p className={`font-display text-[16px] font-semibold ${ui.title}`}>Nội dung Markdown</p><p className={`mt-1 text-[12px] ${ui.body}`}>Chèn ảnh từ Media Service bằng content URL.</p></div><Button type="button" size="icon" variant="ghost" aria-label="Chèn media" disabled={disabled} onClick={() => setLibraryOpen(true)}><Icon icon={ImagePlus} /></Button></div>
      <textarea ref={textareaRef} id={id} value={value} disabled={disabled} placeholder={placeholder} onSelect={(event) => { selectionRef.current = { start: event.currentTarget.selectionStart, end: event.currentTarget.selectionEnd } }} onChange={(event) => { selectionRef.current = { start: event.target.selectionStart, end: event.target.selectionEnd }; onChange(event.target.value) }} className={`${ui.control} mt-4 min-h-48 resize-y py-2`} />
