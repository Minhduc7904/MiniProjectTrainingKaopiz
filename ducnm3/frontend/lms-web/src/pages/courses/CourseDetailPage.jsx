...
export function CourseDetailPage() {
  ...
  if (detail.loading) return <main className={`min-h-svh p-8 ${adminUi.page}`}><LoadingState label="Đang tải khóa học..." /></main>
  if (detail.error) return <main className={`min-h-svh p-8 ${adminUi.page}`}><EmptyState title="Không tải được khóa học" description={detail.error.message} /></main>
  ...
  return (
    <main className={`h-screen min-h-screen overflow-y-auto px-5 py-6 md:px-8 md:py-8 ${adminUi.page}`}>
            <div className="mx-auto max-w-6xl">
              <Link to={APP_ROUTES.courses} className={`inline-flex items-center gap-2 text-[13px] ${adminUi.body}`}><Icon icon={ArrowLeft} />Danh sách khóa học</Link>
              <section className="mt-6 grid items-start gap-7 lg:grid-cols-[minmax(220px,300px)_minmax(0,1fr)]">
                <div className={`group relative overflow-hidden rounded-lg ${adminUi.card}`}>
                  <div className="max-h-72 bg-surface-muted">
                    {course.thumbnail?.contentUrl ? <MediaImagePreview contentUrl={course.thumbnail.contentUrl} alt={course.name} /> : <button type="button" className={`flex h-41.5 flex-1 w-full flex-col items-center justify-center gap-2 ${adminUi.body}`} onClick={() => openMedia('thumbnail')}><Icon icon={ImagePlus} size={28} /><span className="text-[12px]">Thêm thumbnail</span></button>}
                  </div>
              ...
              <section className="mt-10">
                ...
                <div className="mt-4 grid gap-4 lg:grid-cols-[190px_minmax(0,1fr)]">
                  <div className="flex gap-2 overflow-x-auto pb-2 lg:max-h-[390px] lg:flex-col lg:overflow-y-auto lg:pr-2">
                    {gallery.length ? gallery.map((media, index) => <div key={media.usageId} draggable={!savingMedia} onDragStart={() => setDraggedGalleryIndex(index)} onDragEnd={() => setDraggedGalleryIndex(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropGalleryMedia(index)} onClick={() => setSelectedGalleryIndex(index)} className={`group relative shrink-0 cursor-grab overflow-hidden rounded-md border transition ${draggedGalleryIndex === index ? 'scale-95 border-accent opacity-50 ring-2 ring-accent/40' : selectedGallery?.usageId === media.usageId ? 'border-accent ring-2 ring-accent/30' : 'border-line'} bg-surface-muted active:cursor-grabbing`}><div className="lg:aspect-video"><MediaImagePreview contentUrl={media.thumbnailUrl || media.contentUrl} alt="" /></div><span className="absolute left-2 top-2 rounded bg-black/55 px-1.5 py-0.5 text-[10px] text-white">#{index + 1}</span><button type="button" aria-label="Gỡ ảnh khỏi gallery" disabled={savingMedia} onClick={(event) => { event.stopPropagation(); void removeGalleryMedia(media.usageId) }} className="absolute right-1.5 top-1.5 rounded-full bg-black/60 p-1 text-white opacity-100 transition group-hover:scale-105"><Icon icon={Trash2} size={13} /></button><button type="button" aria-label="Xóa ảnh đang hover" disabled={savingMedia} onClick={(event) => { event.stopPropagation(); void removeGalleryMedia(media.usageId) }} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-black/65 p-2 text-white opacity-0 transition group-hover:opacity-100"><Icon icon={Trash2} size={17} /></button>{draggedGalleryIndex !== null && draggedGalleryIndex !== index ? <span className="pointer-events-none absolute inset-x-2 bottom-2 rounded bg-accent px-2 py-1 text-center text-[10px] font-medium text-white">Thả để đặt tại đây</span> : null}</div>) : <button type="button" onClick={() => openMedia('gallery')} className={`flex h-[96px] w-32 shrink-0 flex-col items-center justify-center gap-2 rounded-md border border-dashed ${adminUi.body} lg:w-full`}><Icon icon={ImagePlus} size={25} /><span className="text-[11px]">Tải ảnh lên</span></button>}
                  </div>
                  <div className={`min-h-[280px] overflow-hidden rounded-lg ${adminUi.card}`}>
                    {selectedGallery ? <a href={selectedGallery.contentUrl} target="_blank" rel="noreferrer" className="block h-full min-h-[280px] bg-surface-muted"><MediaImagePreview contentUrl={selectedGallery.contentUrl} alt={course.name} /></a> : <div className={`flex min-h-[280px] items-center justify-center text-[13px] ${adminUi.body}`}>Chọn ảnh gallery để xem media chính</div>}
                  </div>
                </div>
                {mediaError ? <p className={`mt-3 rounded-md px-3 py-2 text-[12px] ${adminUi.badgeDanger}`}>{mediaError}</p> : null}
              </section>

              <section className="mt-10 pb-8"><div className="flex items-end justify-between gap-4"><div><p className={`text-[11px] font-medium tracking-[0.2em] uppercase ${adminUi.eyebrow}`}>Course content</p><h2 className={`mt-1 font-display text-[22px] font-semibold ${adminUi.title}`}>Lessons</h2><p className={`mt-1 text-[12px] ${adminUi.caption}`}>Chọn lesson để xem nội dung; kéo biểu tượng chấm để đổi thứ tự.</p></div><Button type="button" onClick={() => { setLessonError(null); setLessonPanelOpen(true) }}><Icon icon={Plus} />Thêm lesson</Button></div><div className="mt-4 grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]"><aside className={`overflow-hidden rounded-lg ${adminUi.card}`} aria-label="Danh sách lesson">{course.lessons.map((lesson, index) => <div key={lesson.id} onDragOver={(event) => event.preventDefault()} onDrop={() => void reorderLessons(index)} className={`flex items-center gap-1 border-b border-line last:border-b-0 ${selectedLessonId === lesson.id ? 'bg-accent-soft' : ''}`}><button type="button" draggable={!reorderingLessons} aria-label={`Kéo ${lesson.title} để đổi thứ tự`} onDragStart={() => setDraggedLessonIndex(index)} onDragEnd={() => setDraggedLessonIndex(null)} className={`cursor-grab p-3 ${adminUi.caption} active:cursor-grabbing ${draggedLessonIndex === index ? 'opacity-40' : ''}`}><Icon icon={GripVertical} size={17} /></button><button type="button" onClick={() => setSelectedLessonId(lesson.id)} className={`min-w-0 flex-1 px-1 py-3 text-left ${selectedLessonId === lesson.id ? adminUi.title : adminUi.body}`}><span className={`mr-2 font-mono text-[11px] ${adminUi.caption}`}>{String(index + 1).padStart(2, '0')}</span><span className="text-[13px] font-medium">{lesson.title}</span></button></div>)}{!course.lessons.length ? <p className={`p-4 text-[13px] ${adminUi.body}`}>Chưa có lesson.</p> : null}</aside><div className={`min-h-[360px] rounded-lg p-5 ${adminUi.card}`}>{selectedLessonLoading ? <LoadingState label="Đang tải nội dung lesson..." /> : selectedLessonDetail ? <><div className="flex items-start justify-between gap-3"><div><p className={`text-[11px] font-medium tracking-[0.18em] uppercase ${adminUi.eyebrow}`}>Lesson {String(course.lessons.findIndex((lesson) => lesson.id === selectedLessonId) + 1).padStart(2, '0')}</p><h3 className={`mt-1 font-display text-[20px] font-semibold ${adminUi.title}`}>{selectedLessonDetail.title}</h3></div><div className="flex shrink-0 items-center gap-2"><Button type="button" size="sm" variant="ghost" onClick={() => void openLessonEditor(selectedLessonDetail.id)}><Icon icon={Pencil} size={15} />Sửa</Button><Button type="button" size="sm" variant="danger" onClick={() => setConfirm(createLessonDeleteConfirmation(selectedLessonDetail))}><Icon icon={Trash2} size={15} />Xóa lesson</Button></div></div><div className={`mt-5 whitespace-pre-wrap text-[14px] leading-6 ${adminUi.body}`}>{selectedLessonDetail.contentMarkdown || 'Lesson chưa có nội dung.'}</div><div className={`mt-7 pt-5 ${adminUi.hairlineT}`}><div className="flex items-center justify-between gap-3"><div><p className={`text-[11px] font-medium tracking-[0.18em] uppercase ${adminUi.eyebrow}`}>Media đính kèm</p><p className={`mt-1 text-[12px] ${adminUi.caption}`}>Hỗ trợ ảnh, video, audio, tài liệu và tệp khác.</p></div><Button type="button" size="sm" disabled={lessonMediaSaving} onClick={() => setLessonMediaOpen(true)}><Icon icon={ImagePlus} size={15} />Thêm tài liệu</Button></div>{selectedLessonDetail.attachments?.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{selectedLessonDetail.attachments.map((media) => { const MediaIcon = lessonMediaIcon(media.mediaType); const previewUrl = media.thumbnailUrl || (media.mediaType === 'IMAGE' ? media.contentUrl : null); return <article key={media.usageId} className={`group relative overflow-hidden rounded-md ${adminUi.choiceIdle}`}><button type="button" className="w-full cursor-pointer text-left" aria-label={`Xem trước ${media.originalFileName}`} onClick={() => setPreviewMedia(media)}><div className="aspect-video bg-surface-muted">{previewUrl ? <MediaImagePreview contentUrl={previewUrl} alt={media.originalFileName} /> : <div className={`flex h-full flex-col items-center justify-center gap-2 ${adminUi.caption}`}><Icon icon={MediaIcon} size={30} /><span className="text-[11px]">{media.mediaType}</span></div>}</div><div className="min-w-0 p-3"><p className={`truncate text-[12px] font-medium ${adminUi.title}`}>{media.originalFileName}</p><p className={`mt-1 text-[11px] ${adminUi.caption}`}>{media.mediaType} · {media.contentType}</p></div></button><button type="button" aria-label={`Gỡ ${media.originalFileName}`} disabled={lessonMediaSaving} onClick={(event) => { event.stopPropagation(); void removeLessonMedia(media.usageId) }} className={`absolute right-2 top-2 rounded-full p-1.5 opacity-0 transition group-hover:opacity-100 focus:opacity-100 ${adminUi.mediaAction}`}><Icon icon={Trash2} size={14} /></button></article> })}</div> : <p className={`mt-4 rounded-md border border-dashed p-4 text-[13px] ${adminUi.body}`}>Chưa có media đính kèm.</p>}{lessonMediaError ? <p className={`mt-3 rounded-md px-3 py-2 text-[12px] ${adminUi.badgeDanger}`}>{lessonMediaError}</p> : null}</div></> : <EmptyState title="Chọn một lesson" description="Nội dung và media đính kèm sẽ hiển thị tại đây." />}</div></div></section>
            </div>
      <RightPanel open={lessonPanelOpen} title="Thêm lesson" description={`Lesson mới sẽ được thêm vào cuối khóa học.`} onClose={() => { if (!lessonSaving) setLessonPanelOpen(false) }} footer={<div className="flex justify-end gap-2"><Button type="button" variant="ghost" disabled={lessonSaving} onClick={() => setLessonPanelOpen(false)}>Hủy</Button><Button type="submit" form="create-lesson-form" disabled={lessonSaving || !lessonForm.title.trim()}>Lưu lesson</Button></div>}>
        <form id="create-lesson-form" className="space-y-4" onSubmit={submitLesson}>
          <label className={`block text-[13px] ${adminUi.body}`}>Tiêu đề lesson<input className="mt-1 w-full rounded-md border border-line bg-transparent px-3 py-2 text-[14px]" maxLength={200} value={lessonForm.title} onChange={(event) => setLessonForm((current) => ({ ...current, title: event.target.value }))} placeholder="Ví dụ: Tổng quan về hệ thống" /></label>
          ...
          {lessonError ? <p className={`rounded-md px-3 py-2 text-[12px] ${adminUi.badgeDanger}`}>{lessonError}</p> : null}
        </form>
      </RightPanel>
      <RightPanel open={coursePanelOpen} title="Chỉnh sửa khóa học" description="Chỉ các trường đã thay đổi mới được gửi." onClose={() => { if (!courseSaving) setCoursePanelOpen(false) }} footer={<div className="flex justify-end gap-2"><Button type="button" variant="ghost" disabled={courseSaving} onClick={() => setCoursePanelOpen(false)}>Hủy</Button><Button type="submit" form="update-course-form" disabled={courseSaving}>Lưu thay đổi</Button></div>}>
        <form id="update-course-form" className="space-y-4" onSubmit={submitCourseUpdate}>
          <label className={`block text-[13px] ${adminUi.body}`}>Tên khóa học<input className={`${adminUi.control} mt-1 w-full py-2`} maxLength={200} value={courseForm.name} onChange={(event) => setCourseForm((current) => ({ ...current, name: event.target.value }))} /></label>
          ...
          {courseError ? <p className={`rounded-md px-3 py-2 text-[12px] ${adminUi.badgeDanger}`}>{courseError}</p> : null}
        </form>
      </RightPanel>
      <RightPanel open={lessonEdit !== null} title="Chỉnh sửa lesson" description={lessonEdit?.loading ? 'Đang tải chi tiết lesson...' : 'Chỉ các trường đã thay đổi mới được gửi.'} onClose={() => { if (!lessonEditSaving) setLessonEdit(null) }} footer={lessonEdit?.loading ? null : <div className="flex justify-end gap-2"><Button type="button" variant="danger" disabled={lessonEditSaving} onClick={() => setConfirm(createLessonDeleteConfirmation({ id: lessonEdit.id, title: lessonEdit.form.title }))}>Xóa</Button><Button type="button" variant="ghost" disabled={lessonEditSaving} onClick={() => setLessonEdit(null)}>Hủy</Button><Button type="submit" form="update-lesson-form" disabled={lessonEditSaving}>Lưu thay đổi</Button></div>}>
        {lessonEdit?.loading ? <LoadingState label="Đang tải lesson..." /> : lessonEdit?.form ? <form id="update-lesson-form" className="space-y-4" onSubmit={submitLessonUpdate}>
          <label className={`block text-[13px] ${adminUi.body}`}>Tiêu đề lesson<input className={`${adminUi.control} mt-1 w-full py-2`} maxLength={200} value={lessonEdit.form.title} onChange={(event) => setLessonEdit((current) => ({ ...current, form: { ...current.form, title: event.target.value } }))} /></label>
          ...
          {lessonEditError ? <p className={`rounded-md px-3 py-2 text-[12px] ${adminUi.badgeDanger}`}>{lessonEditError}</p> : null}
        </form> : null}
      </RightPanel>
      <MediaLibraryModal
        open={mediaMode !== null}
    ...
    <>
        ...
        <InputPanel
          guided={
            <div className="flex h-full min-h-0 flex-col">
              ...
              <div className={`shrink-0 px-5 py-3 ${adminUi.hairlineT}`}><Pagination page={list.query.page} totalPages={list.pagination.totalPages} totalItems={list.pagination.totalItems} pageSize={list.query.pageSize} loading={list.loading} itemLabel="khóa học" onPageChange={(page) => load({ page })} onPageSizeChange={(pageSize) => load({ page: 1, pageSize })} /></div>
            </div>
          }
          manual={<pre className={`m-5 overflow-auto rounded-md p-3 ${adminUi.code}`}>{JSON.stringify(list.query, null, 2)}</pre>}
        />
      }
      output={
        <OutputPanel json={{ data: list.data, error: list.error, meta: { pagination: list.pagination, traceId: list.traceId, query: list.query } }} activity={GET_COURSES_ACTIVITY} run={list}>
          ...
          {list.data.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{list.data.map((course) => <Link key={course.id} to={APP_ROUTES.courseDetails.replace(':courseId', course.id)} className={`overflow-hidden rounded-lg ${adminUi.card}`}><div className="aspect-video bg-surface-muted">{course.thumbnailUrl ? <MediaImagePreview contentUrl={course.thumbnailUrl} alt={course.name} /> : <div className={`flex h-full items-center justify-center text-[12px] ${adminUi.caption}`}>Chưa có thumbnail</div>}</div><div className="p-4"><h2 className={`truncate text-[15px] font-medium ${adminUi.title}`}>{course.name}</h2><p className={`mt-2 text-[12px] ${adminUi.body}`}>{course.status}</p></div></Link>)}</div> : null}
        </OutputPanel>
      }
      />
      <RightPanel open={coursePanelOpen} title="Tạo khóa học" description="Khóa học mới sẽ được lưu ở trạng thái Bản nháp." onClose={() => { if (!courseSaving) setCoursePanelOpen(false) }} footer={<div className="flex justify-end gap-2"><Button type="button" variant="ghost" disabled={courseSaving} onClick={() => setCoursePanelOpen(false)}>Hủy</Button><Button type="submit" form="create-course-form" disabled={courseSaving || !isCourseCreateFormValid(courseForm)}>Tạo khóa học</Button></div>}>
        <form id="create-course-form" className="space-y-4" onSubmit={submitCourseCreate}>
          <label className={`block text-[13px] ${adminUi.body}`}>Tên khóa học<input className={`${adminUi.control} mt-1 w-full py-2`} maxLength={200} disabled={courseSaving} value={courseForm.name} onChange={(event) => setCourseForm((current) => ({ ...current, name: event.target.value }))} placeholder="Ví dụ: Backend Fundamentals" /></label>
