---
name: kz-analyze-token
description: "Analyze token usage + USD cost for an editor session (opencode | cursor | claude), with a Kaopiz DevKit cost breakdown."
---

> DevKit agent command `kz-analyze-token`, published as a skill because Codex has no repo-level slash commands.
...
Sync → **`.agents/commands/kz-analyze-token.md`** / **`.claude/commands/kz-analyze-token.md`**
