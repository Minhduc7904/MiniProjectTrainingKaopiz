...
public sealed record GetStudentsQuery
{
    ...
    private static string? NormalizeStatus(string? status, List<ApiErrorDetail> details)
    {
        ...
        if (StudentStatuses.IsSupported(normalized)) return normalized;
    ...
    {
        if (string.IsNullOrWhiteSpace(sortBy)) return StudentSortField.CreatedAt;
    ...
    private static StudentSortField AddInvalidSort(List<ApiErrorDetail> details)
    {
        details.Add(new ApiErrorDetail("sortBy", "Sort field must be createdAt, displayName, or email."));
        return StudentSortField.CreatedAt;
    }

    private static bool NormalizeDirection(string? sortDirection, List<ApiErrorDetail> details)
    {
        if (string.IsNullOrWhiteSpace(sortDirection)) return true;
    ...
    private static bool AddInvalidDirection(List<ApiErrorDetail> details)
    {
