# Project Phases

Bộ tài liệu này ghi nhận các artefact theo phase của Mini LMS. Phase 0–2 ở mức
lead; Phase 3 bổ sung requirement chi tiết. Basic Design và testcase vẫn thuộc
Phase 4.

- [Phase 0 — Initiation](0/initiation.md)
- [Phase 1 — Kickoff](1/kickoff.md)
- [Assumptions](1/registers/assumptions.md)
- [Risks](1/registers/risks.md)
- [Phase 2 — Planning](2/planning.md)
- [Requirement traceability Phase 0–2](2/traceability.md)
- [Phase 3 — Requirements](3/README.md)
- [Phase 4 — Basic Design + Testcase](4/README.md)
- [Phase 5 — Development + Self-test](5/README.md)

> [!NOTE]
> Phase 3 mô tả hệ thống **phải làm gì**. API, database, sequence chi tiết và
> testcase bước-đến-bước thuộc Phase 4 — Design + Testcase.
