...
public sealed class GetMediaUsageUrlsHandlerTests
{
    ...
    public void HandleAsyncOwnerIdEmptyThrowsValidationError()
    {
        ...
        // Act
        var exception = Assert.ThrowsAsync<MediaService.Application.MediaApplicationException>(
            () => sut.HandleAsync(
                new GetMediaUsageUrlsQuery("STUDENT", "STUDENT_AVATAR", "AVATAR", Guid.Empty),
                TestContext.CurrentContext.CancellationToken));

        // Assert
        Assert.That(exception!.ErrorCode, Is.EqualTo(MediaService.Application.MediaErrorCodes.InvalidMedia));
    }

    private static MediaService.Application.Abstractions.Persistence.MediaUsageUrlRecord CreateRecord(
        Guid usageId,
        uint displayOrder) =>
        new(
            new(
    ...
    private sealed class StubMediaUrlProvider : IMediaUrlProvider
    {
        public Task<MediaUrl> GenerateAsync(
            MediaService.Application.Abstractions.Persistence.MediaRecord media,
            CancellationToken cancellationToken) =>
            Task.FromResult(new MediaUrl("/media/api/media/content", null));

        public Task<IReadOnlyList<MediaUrl>> GenerateManyAsync(
            IReadOnlyList<MediaService.Application.Abstractions.Persistence.MediaRecord> media,
            CancellationToken cancellationToken) =>
