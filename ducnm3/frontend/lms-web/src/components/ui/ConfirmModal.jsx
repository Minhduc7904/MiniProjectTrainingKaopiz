...
const modes = {
  info: { icon: Info, tone: ui.confirmInfo },
  warning: { icon: AlertTriangle, tone: ui.confirmWarning },
  delete: { icon: Trash2, tone: ui.confirmDelete },
}
...
export function ConfirmModal({ open, mode = 'info', title, description, confirmLabel = 'Xác nhận', busy = false, onConfirm, onClose }) {
  ...
  return <dialog ref={dialogRef} className={ui.modal} onCancel={(event) => { event.preventDefault(); if (!busy) onClose() }}>
    <div className="p-5">
      <div className="flex gap-3">
        ...
        <div><h2 className={`text-[17px] font-semibold ${ui.title}`}>{title}</h2><p className={`mt-1 text-[14px] leading-6 ${ui.body}`}>{description}</p></div>
      </div>
    </div>
    <div className={`flex justify-end gap-2 px-5 py-3 ${ui.hairlineT}`}>
