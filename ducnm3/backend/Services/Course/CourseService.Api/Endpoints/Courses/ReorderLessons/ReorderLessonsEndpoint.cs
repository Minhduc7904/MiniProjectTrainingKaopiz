...
public static class ReorderLessonsEndpoint
{
    public static RouteHandlerBuilder MapReorderLessons(this IEndpointRouteBuilder endpoints) =>
        ...
        }).WithName("reorder-course-lessons").WithTags(ServiceNames.Course).Accepts<ReorderLessonsRequest>("application/json").Produces(StatusCodes.Status200OK).Produces<ApiErrorResponse>(StatusCodes.Status403Forbidden).RequireActor(ActorAccess.Admin);
}
