...
1. Đọc toàn bộ `skills/test-unit/reference.md`.
2. Đọc toàn bộ `skills/test-unit/template.md`.
...
Nếu hành vi cần `TestServer`, chuyển sang `skills/test-component`. Nếu cần
MySQL, MinIO, RabbitMQ hoặc migration thật, chuyển sang
`skills/test-integration`; không nới lỏng ranh giới của unit test.
