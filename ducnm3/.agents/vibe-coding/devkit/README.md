...
| ---- | --------------------------------------------- | ---------------------------------------------------------- |
| Skills | `.agents/skills/<id>/` contains `.devkit-managed.json` | Same path **without** that marker, or use `.vibe/skills/<id>/` per your stack |
| Rules | Filenames listed under `rules` in the summary | Other `.md` files in `.agents/rules/` |
