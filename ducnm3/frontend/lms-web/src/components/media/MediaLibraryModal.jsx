  ...
  return createPortal(
    ...
    >
      <header className={`flex items-start justify-between gap-4 px-5 py-4 ${ui.hairlineB}`}>
        <div>
          <p id="media-library-title" className={`font-display text-[18px] font-semibold ${ui.title}`}>Thư viện media</p>
          <p className={`mt-1 text-[13px] ${ui.body}`}>Chọn media đã tải lên hoặc tải media mới.</p>
        </div>
        <Button type="button" variant="ghost" size="icon" aria-label="Đóng" onClick={close}><Icon icon={X} /></Button>
      </header>

      <div className={`px-5 pt-3 ${ui.hairlineB}`}>
        <Tabs
          tabs={showUpload
      ...
      {tab === 'library' ? (
        <div className="flex min-h-0 flex-1 flex-col md:flex-row">
          <aside className={`shrink-0 border-b p-3 md:w-44 md:border-b-0 md:border-r ${ui.hairlineB}`}>
            <p className={`px-2 pb-2 text-[11px] font-medium tracking-[0.18em] uppercase ${ui.eyebrow}`}>Loại media</p>
            <div className="flex gap-1 overflow-x-auto md:flex-col">
              {LIBRARY_TABS.map((item) => (
                <button
                  key={item.id || 'all'}
                  type="button"
                  className={`whitespace-nowrap rounded-md px-3 py-2 text-left text-[13px] ${mediaType === item.id ? ui.navActive : ui.navIdle}`}
          ...
          <section className="min-h-0 min-w-0 flex-1 overflow-y-auto p-4">
            <div className="mb-4 flex items-center justify-between gap-3">
              <p className={`text-[12px] ${ui.body}`}>{library.data.length} media đã tải</p>
              <div className={`flex items-center gap-1 rounded-md p-1 ${ui.choiceIdle}`} role="group" aria-label="Kiểu hiển thị">
                <Button
                  type="button"
            ...
            {library.error ? <p className={`rounded-md px-3 py-2 text-[13px] ${ui.badgeDanger}`}>{library.error.message}</p> : null}
            {library.loading && !library.data.length ? <div className={`flex h-48 items-center justify-center ${ui.body}`}><Icon icon={LoaderCircle} className="animate-spin" /></div> : null}
            {!library.loading && !library.data.length && !library.error ? <p className={`flex h-48 items-center justify-center text-[13px] ${ui.body}`}>Chưa có media phù hợp.</p> : null}
            {viewMode === 'grid' ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
              ...
              <div className={`overflow-hidden rounded-md ${ui.card}`}>
                <div className={`grid grid-cols-[minmax(0,2fr)_100px_120px_100px_170px_48px] gap-3 px-3 py-2 text-[10px] font-medium tracking-[0.14em] uppercase ${ui.tableHead}`}>
        ...
        <form className="min-h-[420px] p-5" onSubmit={submitUpload}>
          <div className={`rounded-md p-4 ${ui.choiceIdle}`}>
            <p className={`font-display text-[14px] font-semibold ${ui.title}`}>Upload direct</p>
            <p className={`mt-1 text-[12px] ${ui.body}`}>Backend tự nhận diện loại media và chọn bucket phù hợp.</p>
            ...
            {upload.error ? <p className={`mt-3 rounded-md px-3 py-2 text-[13px] ${ui.badgeDanger}`}>{upload.error.message}</p> : null}
            {upload.loading ? <p className={`mt-3 flex items-center gap-2 text-[13px] ${ui.body}`}><Icon icon={LoaderCircle} className="animate-spin" />Đang upload...</p> : null}
          </div>
